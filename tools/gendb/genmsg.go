...
type Struct struct {
	...
	KeyIsString    string   `xml:"keyisstring,attr"`
	Type    string   `xml:"type,attr"`
	Comment string   `xml:"comment,attr"`
...
func is_int_type(str string) string {
	if str == "int8_t" ||
		str == "uint8_t" ||
		str == "int16_t" ||
		str == "uint16_t" ||
...
func is_int64_type(str string) string {
	if str == "int64_t" ||
		str == "uint64_t" {
		return "yes"
	}
...
func is_float_type(str string) string {
	if str == "float" {
		return "yes"
	}
	return ""
...
func is_user_type(str string) string {
	if str != "char" &&
		str != "int8_t" &&
		str != "uint8_t" &&
		str != "int16_t" &&
